...
const ProjectSprints = () => {
  ...
  const getSprints = useCallback(async () => {
    try {
      if (releaseId) {
        const sprints = await getSprintsByReleaseId(parseInt(releaseId));
        setSprints(sprints);
        console.log(sprints);
        return sprints;
      }
        ...
        <div>
                ...
                >
                  <td className="w-[30px]">
                    <img
                      ...
                      src={"/src/assets/icons/sprint-icon.png"}
                    />
                  </td>
                  <td className="gap-[10px] text-left">{sprint.name}</td>
          ...
          {sprints.length === 0 && (
            <div className="text-[18px] h-[40px] w-full text-center mt-2">
              <span className="text-center">There are no sprints yet</span>
            </div>
          )}
        </div>
      </Page>
