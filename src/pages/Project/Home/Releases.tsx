...
const ProjectReleases = () => {
  ...
  const getReleases = useCallback(async () => {
    try {
      if (projectId) {
        const releases = await getReleasesByProjectId(parseInt(projectId));
        ...
        <div>
                ...
                >
                  <td className="w-[30px]">
                    <img
                      ...
                      src={"/src/assets/icons/release-icon.png"}
                    />
                  </td>
                  <td className="gap-[10px] text-left">{release.name}</td>
          ...
          {releases.length === 0 && (
            <div className="text-[18px] h-[40px] w-full text-center mt-2">
              <span className="text-center">There are no releases yet</span>
            </div>
          )}
        </div>
      </Page>
